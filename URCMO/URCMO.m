        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population{1})
                ...
                if Problem.FE < first_FES
                    for i = 1 : 2
                        valOffspring{i}(1:ceil(Problem.N/2))             = GA_TournamentSelection(Problem, Population{i}, Fitness{i}, ceil(Problem.N/2));
                        valOffspring{i}(1+ceil(Problem.N/2) : Problem.N) = DE_current_to_rand_1(Problem, Population{i}, floor(Problem.N/2));
                    end
                    ...
                    succ_jilu(cnt,1) = sum(succ1(1:ceil(Problem.N/2)));
                    succ_jilu(cnt,2) = sum(succ1(1 + ceil(Problem.N/2) : Problem.N));
                    
                    succ_jilu(cnt,3) = sum(succ2(1:ceil(Problem.N/2)));
                    succ_jilu(cnt,4) = sum(succ2(1 + ceil(Problem.N/2) : Problem.N)); % only succ2 is used in the paper
                    ...
                    valOffspring{1}(1:ceil(Problem.N/2))             = GA_TournamentSelection(Problem, Population{1}, Fitness{1}, ceil(Problem.N/2));
                    valOffspring{1}(1+ceil(Problem.N/2) : Problem.N) = DE_current_to_rand_1(Problem, Population{1}, floor(Problem.N/2));
                    
                    if flag == 1
                        valOffspring{2}(1:ceil(Problem.N/2))              = GA_TournamentSelection(Problem, Population{2}, Fitness{2}, ceil(Problem.N/2));
                        valOffspring{2}( 1+ceil(Problem.N/2) : Problem.N) = DE_transfer(Problem, Population{2}, Population{1}, floor(Problem.N/2));
                    elseif flag == 2
